...
- 默认的包结构
	- <font color=blue size=4 ><b>主程序所在包及其下面的所有子包里面的组件都会被默认扫描进来</b></font>
	- 无需以前的包扫描配置
	- 想要改变扫描路径
...
- 基本使用
	- <font color=blue size=4><b>Full模式与Lite模式</b></font>
	- 示例
...
@ComponentScan 在[Spring Boot特点-自动配置特性](#)有用例。


@Import({User.class, DBHelper.class})给容器中**<font size=4 color=blue>自动创建出这两个类型的组件</font>**、默认组件的名字就是全类名
...
**<font size=4 color=blue>条件装配：满足Conditional指定的条件，则进行组件注入</font>**
...
<font size=4 color=blue><b>如何使用Java读取到properties文件中的内容，并且把它封装到JavaBean中，以供随时使用</b></font>
...
public class DispatcherServlet extends FrameworkServlet {
    ...
    private void initHandlerAdapters(ApplicationContext context) {
        ...
        if (this.detectAllHandlerAdapters) {// 检测所有处理程序适配器
            //在ApplicationContext中查找所有HandlerAdapters，包括祖先上下文
